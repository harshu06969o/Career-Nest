...
import {
  ...
  Sparkles, ChevronDown, ChevronUp, RefreshCw, ToggleLeft,
  Search, Mail,
  FileText
} from 'lucide-react';
...
export default function RecruiterDashboard() {
  ...
  // Track which job's applicants are currently being viewed
  ...
  const mockApplicants: Applicant[] = [
    { id: '1', name: 'Rahul Sharma', college: 'IIT Bombay', cgpa: 9.2, matchScore: 94, status: 'PENDING' },
    { id: '2', name: 'Priya Patel', college: 'NIT Trichy', cgpa: 8.8, matchScore: 82, status: 'PENDING' },
    { id: '3', name: 'Arjun Singh', college: 'BITS Pilani', cgpa: 7.9, matchScore: 68, status: 'PENDING' },
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="text-left border-b border-gray-200 pb-6">
        ...
        <p className="text-gray-500 mt-1 text-sm">
          Manage job postings and review AI-scored applicants
        </p>
      </div>
            ...
            {jobs.map((job) => (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex gap-6 mt-4 text-sm text-gray-600 border-t border-gray-100 pt-4">
                    ...
                    <button
                      onClick={() => setViewingApplicantsFor((id) => id === job.id ? null : job.id)}
                      className="ml-auto flex items-center gap-1.5 text-indigo-600 font-semibold hover:text-indigo-800 transition-colors"
                    >
                      <Users size={16} />
                  ...
                  <div className="bg-gray-50 border-t border-gray-200 p-6 animate-slide-up">
                    ...
                    <div className="space-y-3">
                      {mockApplicants.map((applicant) => (
                        <div key={applicant.id} className="bg-white border border-gray-200 rounded-lg p-4 flex items-center justify-between shadow-sm">
                          <div className="flex items-center gap-5">
                            <div className="flex-shrink-0">
                              <CircularProgress score={applicant.matchScore} size={56} stroke={5} />
                            </div>
                            <div>
                              <p className="font-bold text-gray-900">{applicant.name}</p>
                              <p className="text-sm text-gray-500">{applicant.college} · {applicant.cgpa} CGPA</p>
                            </div>
                          </div>

                          <div className="flex items-center gap-3">
                            <button className="px-4 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 text-sm font-semibold rounded-lg transition-colors flex items-center gap-2">
                              <FileText size={14} /> Resume
                            </button>
                            <button className="p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Contact Student">
                              <Mail size={18} />
                            </button>
                          </div>
                        </div>
